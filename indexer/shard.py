...
Each of the N parallel shard jobs chunks every repo pinned in the shas file
and embeds only its stride (pending[i::n]) — balanced by construction, no
...
# Same cap as the incremental indexer (indexer/__main__.py): long sequences
# blow up CPU attention memory/time quadratically.
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--shas", required=True, help="JSON file: {repo: commit_sha}")
    ...
    for name in sorted(shas):
        ...
        print(f"[{name}] {sha[:8]} {len(take)}/{len(pending)} chunks for shard {i}/{n}",
              flush=True)

    print(f"shard {i}/{n}: embedding {len(mine)} chunks", flush=True)
    if mine:
        ...
        texts = [f"{repo}/{path}\n{ch.text[:EMBED_MAX_CHARS]}"
                 for repo, path, ch, _ in mine]
        parallel = args.workers if len(mine) >= 1000 else None
        done, t0 = 0, time.time()
        for (repo, path, ch, lang), vec in zip(
            mine, model.embed(texts, batch_size=args.batch_size, parallel=parallel),
        ):
            dbm.insert_chunk(db, repo, path, ch, lang, vec)
            done += 1
            if done % 500 == 0:
                rate = done / (time.time() - t0)
                print(f"  {done}/{len(mine)} ({rate:.0f}/s)", flush=True)
                db.commit()

    dbm.set_meta(db, "updated_at", now)
    db.commit()
    print(f"shard {i}/{n} done: {len(mine)} chunks", flush=True)
    db.close()
    return 0
